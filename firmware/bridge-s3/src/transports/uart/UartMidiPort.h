...
/**
 * @brief MIDI-over-UART MidiTransport (DIN MIDI).
 * 
 ...
 */
class UartMidiPort : public MidiTransport {
public:
    UartMidiPort(HardwareSerial& serial = Serial2, int rxPin = 48, int txPin = 47);
    
    bool begin(uint32_t baud = 31250);
    const char* name() const override { return "UART-MIDI"; }
    MidiTransportKind kind() const override { return MidiTransportKind::kUart; }
