...
void BleMidiPeripheral::processIncomingBlePacket(const uint8_t* data, size_t length)
{
...
BleMidiPeripheral::BleMidiPeripheral()
    : pServer(nullptr),
...
void BleMidiPeripheral::begin(const std::string& deviceName) {
    if (pServer) {
        return;
    }
    ...
    public:
        BleMidiPeripheral* bleCon;
        explicit ServerCallbacks(BleMidiPeripheral* con) : bleCon(con) {}
    ...
    public:
        BleMidiPeripheral* bleCon;
        BLECallback(BleMidiPeripheral* con) : bleCon(con) {}
...
bool BleMidiPeripheral::sendMidi(const uint8_t* data, size_t length) {
...
bool BleMidiPeripheral::sendDebugText(const char* text) {
...
void BleMidiPeripheral::recordForwardLatency(uint32_t latencyMs)
{
    if (avgLatencyMs_ == 0) {
...
bool BleMidiPeripheral::isConnected() const {
    if(pServer)
        return (pServer->getConnectedCount() > 0);
    return false;
}

void BleMidiPeripheral::setMidiMessageCallback(MIDIMessageCallback cb) {
    midiCallback = cb;
}

void BleMidiPeripheral::onMidiDataReceived(const uint8_t* data, size_t length) {
    if (receiveCallback_) {
        receiveCallback_(data, length);
    }
}

bool BleMidiPeripheral::enqueueIncomingMidi(const uint8_t* data, size_t length)
{
...
bool BleMidiPeripheral::dequeueIncomingMidi(RawBleMessage& message)
{
    portENTER_CRITICAL(&incomingMux_);
...
void BleMidiPeripheral::dispatchIncomingMidi(const uint8_t* data, size_t length)
{
    if (midiCallback) {
...
void BleMidiPeripheral::setSubscribed(bool subscribed)
{
