...
public:
    ...
    Result route(MidiTransport* source, const uint8_t* data, size_t length);

    const Counters& counters() const { return counters_; }
    const RouteStats& statsFor(MidiTransportKind kind) const { return counters_.transport[transportKindIndex(kind)]; }

private:
    BridgeUi* ui_ = nullptr;
    std::function<bool()> isPaused_;
    MidiEngine* engine_ = nullptr;
    std::vector<MidiTransport*> transports_;
    Counters counters_;

    void onMidiReceived(MidiTransport* source, const uint8_t* data, size_t length);
    void notifyRouteUi(const uint8_t* data, size_t length, uint16_t color);
    RouteStats& statsForMutable(MidiTransportKind kind) { return counters_.transport[transportKindIndex(kind)]; }
    bool isPaused() const;
};
