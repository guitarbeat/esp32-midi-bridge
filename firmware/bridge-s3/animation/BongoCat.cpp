...
void BongoCatDisplay::draw(Arduino_GFX* gfx, int16_t xOffset)
{
    if (gfx == nullptr) {
        return;
    }

    const int16_t drawX = (xOffset >= 0) ? xOffset : kDefaultOriginX;
    ...
    gfx->draw16bitRGBBitmap(drawX, kOriginY, frameBuffer, 128, 128);
}
